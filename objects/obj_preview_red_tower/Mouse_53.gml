...
budget = obj_money_controller.current_gems.red;
if(!instance_exists(obj_tower_parent) and !place_meeting(x,y,obj_prevent_tower_placement)) {
	if(instance_exists(obj_level_controller)){
		...
		if(cost <= budget){
			obj_money_controller.current_gems.red -= cost;
	...
	instance_destroy(self);
}else if(!place_meeting(x,y,obj_tower_parent) and !place_meeting(x,y,obj_prevent_tower_placement)){
	if(instance_exists(obj_level_controller)){
		// TODO: Change this to gems 
		if(cost <= budget){
			obj_money_controller.current_gems.red -= cost;
